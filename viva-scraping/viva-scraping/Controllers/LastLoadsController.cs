...
{
    public class LastLoadsController : Controller
    {
        public const string LastLoadsURL = "https://www.portalviva.pt/lx/pt/myvivaclient/client-account-area/loads/last-loads.aspx";
        ...
        public async Task<IActionResult> Get()
        {
            ...
            var response = await DataRequester.Get(User, LastLoadsURL);
            ...
            var cardsSelect = doc.DocumentNode.Descendants("select").Where(d => d.Attributes.Contains("id") && d.Attributes["id"].Value.Contains("ContentPlaceHolderDefault_content_AccountViva_Tabs_ClientViva_Loads_Content_Client_LastLoads_11_dropMyCards")).First();
        ...
        {
            var responseGet = await DataRequester.Get(User, LastLoadsURL);
            ...
            LastLoads result = new LastLoads();
            result.loads = new List<Load>();

            var response = await DataRequester.Post(User, LastLoadsURL, nvc);
